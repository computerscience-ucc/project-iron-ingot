...
export async function fetchAllAwards() {
  const query = `
    *[_type == 'award'] | order(_createdAt desc) {
      ...
      "badges": awardBadges,
      "recipients": recipients[] -> { fullName, batchYear, program },
      tags,
      _createdAt
    }
  `;
...
const typeQueryMap = {
  blog: "*[_type == \"blog\"] | order(_createdAt desc, _updatedAt desc) { _id, _createdAt, _updatedAt, _type, \"headerImage\": headerImage.asset -> url, \"title\": blogTitle, \"slug\": slug.current, \"authors\": blogAuthor[] -> { fullName, pronouns, \"authorPhoto\": authorPhoto.asset -> url }, academicYear, tags }",
  bulletin: "*[_type == \"bulletin\"] | order(_createdAt desc, _updatedAt desc) { _id, _createdAt, _updatedAt, _type, \"headerImage\": headerImage.asset -> url, \"title\": bulletinTitle, \"slug\": slug.current, \"authors\": bulletinAuthor[] -> { fullName, pronouns, \"authorPhoto\": authorPhoto.asset -> url }, tags }",
  thesis: "*[_type == \"thesis\"] | order(academicYear desc, _createdAt desc) { _id, _createdAt, _updatedAt, _type, \"headerImage\": headerImage.asset -> url, \"title\": thesisTitle, \"slug\": slug.current, \"authors\": postAuthor[] -> { fullName, pronouns, \"authorPhoto\": authorPhoto.asset -> url }, \"description\": pt::text(thesisContent), academicYear, department, tags }",
  award: "*[_type == \"award\"] | order(academicYear desc, dateAwarded desc) { _id, _createdAt, _updatedAt, _type, \"headerImage\": headerImage.asset -> url, \"title\": awardTitle, \"slug\": slug.current, \"category\": awardCategory, \"badges\": awardBadges, \"recipients\": recipients[] -> { fullName, pronouns, batchYear, yearLevel, program, \"recipientPhoto\": recipientPhoto.asset -> url }, \"images\": awardImages[].asset->url, \"description\": awardDescription, academicYear, dateAwarded, tags }",
  gallery: "*[_type == \"gallery\"] | order(projectDate desc, _createdAt desc) { _id, _createdAt, _updatedAt, _type, \"title\": projectTitle, \"slug\": slug.current, personName, \"profilePicture\": profilePicture.asset -> url, projectDate, youtubeEmbedLink, githubUrl, linkedinProfile, tags }",
};

export function listenSanity(onMutation) {
  const subscription = client.listen('*[_type in ["blog", "bulletin", "thesis", "award", "gallery"]]').subscribe({
    next: (event) => {
      const type = event.result?._type || event.documentId?.split(".")[0];
      if (type && typeQueryMap[type]) {
        onMutation(type);
      }
    },
    error: (err) => console.warn("[sanity listen] Error:", err),
  });
  return subscription;
}

export function refetchType(type) {
...
export async function buildSiteContext() {
  ...
  const theses = results[0].status === "fulfilled" ? results[0].value : [];
  const blogs = results[1].status === "fulfilled" ? results[1].value : [];
  const bulletins = results[2].status === "fulfilled" ? results[2].value : [];
  const awards = results[3].status === "fulfilled" ? results[3].value : [];

  if (results[0].status === "rejected") console.error("[buildSiteContext] Failed to fetch theses:", results[0].reason);
  if (results[1].status === "rejected") console.error("[buildSiteContext] Failed to fetch blogs:", results[1].reason);
  if (results[2].status === "rejected") console.error("[buildSiteContext] Failed to fetch bulletins:", results[2].reason);
  if (results[3].status === "rejected") console.error("[buildSiteContext] Failed to fetch awards:", results[3].reason);
...
`;
  ...
  if (awards.length > 0) {
    ...
    awards.forEach((a, i) => {
      const recipients = a.recipients?.map((r) => r.fullName || "Unknown").join(", ") || "Unknown";
